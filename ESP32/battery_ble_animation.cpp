...
/* ===================== Animation layout ===================== */
// Must match how test.h was generated.
#define FACE_SCALE  4
...
static void drawFaceRegion() {
  ...
  for (int16_t y = 0; y < ANIM_H; y++) {
    for (int16_t x = 0; x < ANIM_W; x++) {
      ...
      int16_t sx = x * FACE_SCALE;
      int16_t sy = y * FACE_SCALE;

      for (int16_t dy = 0; dy < FACE_SCALE; dy++) {
        for (int16_t dx = 0; dx < FACE_SCALE; dx++) {
          tft.writePixel(sx + dx, sy + dy, color);
        }
      }
    }
  }
...
static void drawWatchStatsValues(bool force = false) {
  ...
  // Right column only: icon + percentage, shifted upward
...
static void updateDebugHRonTFT() {
  ...
  if (lastDbgBpm == currentBPM && lastDbgBatt == batteryPercent) return;
